...
std::array<GLfloat, 16> mat4perspective(float vfov, float aspect, float znear, float zfar) {
    float f = cot(vfov / 2.0f);
...
int main(int, char*[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::array<GLfloat, 16> matTrans = translation(0.0f, 0.0f, -3.0f);
        //std::array<GLfloat, 16> matPerspective = mat4perspective(3.14/4, 1.0f, 0.1f, 100.0f);
        std::array<GLfloat, 16> matPerspective = mat4perspective(30.0f * 3.14159f / 180, 16.0f / 9.0f, 0.1f, 100.0f);
        std::array<GLfloat, 16> matTR = matrisMultiplikation(matR1, matTrans);
        matTR = matrisMultiplikation(matTR, matPerspective);    
        ...
        glUseProgram(myShader.id());  // Activate the shader to set its variables
        glUniformMatrix4fv(locationMV, 1, GL_FALSE, matTR.data());  // Copy the value'

        // GLint locationP = glGetUniformLocation(myShader.id(), "P");
        // glUseProgram(myShader.id());  // Activate the shader to set its variables
        // glUniformMatrix4fv(locationP, 1, GL_FALSE, mat1.data());  // Copy the value
