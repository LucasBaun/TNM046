...
int main(int, char*[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::array<GLfloat, 16> matS = scale(0.2f, 0.2f, 0.0f);
        std::array<GLfloat, 16> matOT = translation(0.5f, 0.0f, 0.0f);

        
        std::array<GLfloat, 16> mat1 = matrisMultiplikation(matR1, cam);
        std::array<GLfloat, 16> mat2 = matrisMultiplikation(mat1, matS);
        std::array<GLfloat, 16> mat3 = matrisMultiplikation(translation(4,0,0), mat2);
        std::array<GLfloat, 16> mat4 = matrisMultiplikation(matR2, mat3);
        ...
        glUseProgram(myShader.id());  // Activate the shader to set its variables
        glUniformMatrix4fv(locationT, 1, GL_FALSE, mat4.data());  // Copy the value
